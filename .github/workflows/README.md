...
|------|---------|---------|
| **Fast (per-PR)** | every PR; push to master/main/dev/churn | host unit suites (one check each), Arm PSA-FF conformance, cross-compile (Cortex-M33, and the AArch64 EL3 image with its symbol guard plus the smoke, boot, and boot-smp2 scenarios on QEMU virt/versal-virt), compiler matrix, sanitizers, valgrind, integrations, core/port split guard |
