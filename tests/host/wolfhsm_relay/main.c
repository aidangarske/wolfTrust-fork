...
static void test_guest_init_retry(int init_rc)
{
    WC_RNG rng;
    uint8_t output[32];
...
int main(void)
{
    ...
    g_reject_connect = 1;
    guest_init_rc = test_wolfhsm_sys_init();
    check(guest_init_rc == 0,
          "guest SYS_INIT installs a retry callback after cold-start failure");
    ...
    test_guest_init_retry(guest_init_rc);
